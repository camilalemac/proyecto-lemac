...
export const obtenerCursosPorAlumnos = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    const { alumnoIds } = req.body as { alumnoIds: number[] };
    
    if (!Array.isArray(alumnoIds) || alumnoIds.length === 0) {
      res.status(400).json({ success: false, message: "Debe proveer un arreglo válido de alumnoIds." });
      return;
    }

    const cursosMap = await matriculaService.obtenerCursosPorAlumnos(alumnoIds);
    res.status(200).json({ success: true, data: cursosMap });
  } catch (err) {
    next(err);
  }
};
