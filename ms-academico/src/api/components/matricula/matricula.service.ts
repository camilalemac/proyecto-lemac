...
export const matriculaService = {
  listarAlumnosPorCurso: async (cursoId: number, colegioId: number): Promise<unknown[]> => {
  ...
  listarMatriculasPorAlumno: async (alumnoId: number, colegioId: number): Promise<unknown[]> => {
  ...
  obtenerCursosPorAlumnos: async (alumnoIds: number[]): Promise<Record<number, string>> => {
    const cursosData: any[] = await matriculaRepository.findCursosByAlumnos(alumnoIds);
    
    return cursosData.reduce((acc, row) => {
      acc[row.ALUMNO_ID] = row.CURSO_NOMBRE;
      return acc;
