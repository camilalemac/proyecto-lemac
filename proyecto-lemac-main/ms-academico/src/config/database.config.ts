...
const dbName =
  process.env.DB_NAME || (dialect === "mysql" ? "test" : "softwarepayescolar_academico");
...
if (dialect === "oracle") {
  ...
  if (fs.existsSync(sqlnetFile)) {
    let sqlnetContent = fs.readFileSync(sqlnetFile, "utf8");
    ...
    if (updatedSqlnetContent !== sqlnetContent) {
      ...
      logger.info(`[ms-academico] sqlnet.ora actualizado con WALLET_LOCATION=${walletPath}`);
    }
  }
  ...
  if (!selectedConnectString || !tnEntries.includes(selectedConnectString)) {
    ...
    if (selectedConnectString) {
      logger.warn(
        `[ms-academico] '${selectedConnectString}' no existe en tnsnames.ora, usando '${auto}' en su lugar.`,
      );
    }
  ...
  try {
    ...
    logger.warn("[ms-academico] oracledb.initOracleClient: ", err);
  }
}

logger.info("[ms-academico] Configuración DB:");
logger.info(`  dialect: ${dialect}`);
logger.info(`  dbName: ${dbName}`);
logger.info(`  user: ${dbUser}`);
logger.info(`  walletPath: ${walletPath}`);
logger.info(`  selectedConnectString: ${selectedConnectString}`);

const sequelizeOptions: Record<string, unknown> = {
  dialect: dialect as "oracle" | "mysql",
  logging: (sql: string, timing?: number) => {
    logger.debug(`[ms-academico][sequelize] ${sql}${timing ? ` (${timing}ms)` : ""}`);
  },
  pool: {
    max: 10,
    min: 0,
