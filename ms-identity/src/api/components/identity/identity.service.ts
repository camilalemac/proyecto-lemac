...
// Agregamos el token como parámetro opcional
export const me = async (userId: number, token?: string) => {
  // 1. Usamos findOne para filtrar manualmente a los usuarios no eliminados
  ...
  // 3. Verificamos si el usuario tiene el rol de alumno
  const esAlumno = rolesFormateados.some((r: any) => r.rol_code === "ALU_REG");
  let cursoAlumno = null;

  // 4. Si es alumno y tenemos token, solicitamos su curso al ms-academico
  if (esAlumno && token) {
    try {
      const rawGatewayUrl = process.env.MS_GATEWAY_URL || "http://localhost:3002/api/v1"; 
      const baseUrl = rawGatewayUrl.replace(/\/$/, "");
      
      const response = await axios.post(
        `${baseUrl}/academico/matriculas/cursos-por-alumnos`, 
        { alumnoIds: [user.userId] }, // Enviamos solo el ID de este alumno
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response.data && (response.data.success === true || response.data.status === "success")) {
        // Extraemos el curso usando el userId como llave
        cursoAlumno = response.data.data[user.userId as number] || "Sin curso asignado";
      }
    } catch (error: any) {
      console.error("Error comunicándose con ms-academico en me():", error.message);
    }
  }

  // 5. Retornamos la estructura perfecta
  return {
    perfil: {
      user_id: user.userId,
      nombres: user.nombres,
    ...
    academico: { 
      cursos_jefatura: [],
      curso: cursoAlumno // <--- AQUÍ SE INYECTA EL CURSO DEL ALUMNO
    },
    finanzas: null,
    seguridad: {
      ultimo_acceso: "18/03/26 23:24:34", // Nota: Esto idealmente deberías dinamizarlo después
      ip: "190.168.1.45",
    },
  };
};
...
export const getMisHijos = async (userId: number, token: string) => {
  // 1. Encontrar al apoderado para obtener su grupoId
  const apoderado = await Identity.findOne({
    where: { userId: userId, estado: "ACTIVO" }
  });
  ...
  if (!apoderado.grupoId) {
    // Si no tiene grupo familiar, no tiene hijos asignados
    return []; 
  }

  // 2. Buscar a todos los usuarios del mismo grupo familiar con rol "ALUMNO"
  const hijos = await Identity.findAll({
    where: { 
      grupoId: apoderado.grupoId,
      estado: "ACTIVO"
    },
    include: [{
      model: Role,
      as: "roles",
      where: { rolCode: { [Op.in]: ["ALU_REG", "DIR_PRES_ALU", "CEN_PRES_CAL", "CEN_TES_CAL", "CEN_SEC_CAL", "STF_DIR"] } } 
    }]
  });

  if (hijos.length === 0) return [];

  // Extraemos los IDs de los alumnos para consultar sus cursos
  const hijosIds = hijos.map((hijo) => hijo.userId as number);

  // 3. Solicitar los cursos al ms-academico a través del ms-gateway
  let cursosMap: Record<number, string> = {};
  try {
    // 1. Obtenemos la URL del Gateway y le quitamos cualquier barra final extra (/) para evitar doble barra
    const rawGatewayUrl = process.env.MS_GATEWAY_URL || "http://localhost:3002/api/v1"; 
    const baseUrl = rawGatewayUrl.replace(/\/$/, "");
    
    // 2. Apuntamos oficialmente a través del Gateway
    const response = await axios.post(
      `${baseUrl}/academico/matriculas/cursos-por-alumnos`, 
      { alumnoIds: hijosIds },
      {
    ...
    // 3. Asignación flexible con los datos
    if (response.data && (response.data.success === true || response.data.status === "success")) {
      cursosMap = response.data.data;
    }
  } catch (error: any) {
    console.error("Error comunicándose con ms-academico mediante gateway:", error.message);
  }

  // 4. Formatear la respuesta final para el Frontend, inyectando el curso
  return hijos.map(hijo => ({
    alumnoId: hijo.userId, 
    nombres: hijo.nombres,
    apellidos: hijo.apellidos,
    rutCompleto: hijo.rutCuerpo && hijo.rutDv ? `${hijo.rutCuerpo}-${hijo.rutDv}` : null,
    email: hijo.email,
    curso: cursosMap[hijo.userId as number] || "Sin curso asignado"
  }));
};

/**
 * Obtiene la información completa de un usuario por su ID primario.
 * Diseñado especialmente para intercomunicación entre microservicios (ej. Cuentas por Cobrar).
 */
export const obtenerUsuario = async (userId: number) => {
  const user = await Identity.findByPk(userId, {
    // Excluimos datos sensibles de autenticación por seguridad informática elemental
    attributes: { exclude: ["passwordHash"] }
  });

  if (!user) {
    throw new ApiError(404, `Usuario con ID ${userId} no encontrado en Identity`);
  }
