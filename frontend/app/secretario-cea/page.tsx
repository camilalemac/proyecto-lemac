...
export default function SecretarioCEADashboard() {
  ...
  const [secretarioName, setSecretarioName] = useState<string>("Secretario(a) CEA");
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 🚨 AJUSTA ESTE ROL según lo que uses para el Secretario (Ej: CEN_SEC_CAL)
      if (payload.role !== "CEN_SEC_CAL" && payload.rol !== "CEN_SEC_CAL") {
        // throw new Error("Acceso denegado: Se requieren permisos de Secretario CEA.");
      }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // 🚀 AQUÍ IRÁ TU LLAMADA AL BACKEND CUANDO LO TENGAS LISTO
      // await fetch(`${API_URL}/endpoint-para-guardar-actas`, { ... })
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-xl tracking-tight text-slate-800">Secretaría CEA</span>
          ...
          <nav className="flex items-center gap-4">
            ...
            {/* BOTÓN OBLIGATORIO: VOLVER A VISTA DE ALUMNO */}
            <button 
      ...
      <main className="flex-grow max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex flex-col h-[calc(100vh-4rem)]">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-t-xl border border-slate-200 shadow-sm border-b-0">
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">
              Redacta actas de reuniones, comunicados oficiales o notas informativas.
            </p>
          </div>
        ...
        <div className="flex-grow relative shadow-sm">
          <textarea
            ...
            placeholder="Comienza a escribir aquí el acta de la reunión o el comunicado..."
