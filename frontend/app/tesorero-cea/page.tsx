...
export default function TesoreroCEADashboard() {
  const router = useRouter();

  const [loading, setLoading] = useState(true);
  const [submitLoading, setSubmitLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [tesoreroName, setTesoreroName] = useState<string>("Tesorero(a) CEA");
  ...
  const [tipoForm, setTipoForm] = useState<"INGRESO" | "EGRESO">("EGRESO"); // Default egreso por ser tesorero
  ...
  const loadData = async () => {
    try {
      ...
      try {
        ...
        // Validamos el rol específico solicitado: CEN_TES_CAL
        if (payload.role !== "CEN_TES_CAL" && payload.rol !== "CEN_TES_CAL") {
          throw new Error("Acceso denegado: Se requieren permisos de Tesorero CEA.");
        }
      ...
      if (resMovimientos?.ok) {
        ...
        // Mapeo seguro con fallbacks para evitar errores si Oracle cambia nombres
        const movimientosMapeados = data.data.map((item: any, index: number) => ({
          id: item.MOVIMIENTO_CEA_ID || item.movimiento_cea_id || item.id || index,
          tipo: item.TIPO_MOVIMIENTO_CEA || item.tipo_movimiento_cea || item.tipo || "EGRESO",
          monto: Number(item.MONTO_MOVIMIENTO_CEA || item.monto_movimiento_cea || item.monto || 0),
          fecha: item.FECHA_MOVIMIENTO_CEA || item.fecha_movimiento_cea || item.fecha || new Date().toISOString(),
          descripcion: item.DESC_MOVIMIENTO_CEA || item.desc_movimiento_cea || item.descripcion || "Sin descripción"
        }));
  ...
  const handleSubmitGasto = async (e: React.FormEvent) => {
    ...
    try {
      const token = Cookies.get("authToken");
      const res = await fetch(`${API_URL}/pagos/movimientoCea/movimientos`, {
        method: "POST",
        headers: {
      ...
      // Recargar la tabla y los resúmenes calculados
      await loadData();
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans text-slate-900 pb-16">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-xl tracking-tight text-slate-800">Tesorería CEA</span>
              <span className="hidden md:inline-block ml-2 text-xs font-semibold bg-orange-50 text-orange-700 px-2.5 py-0.5 rounded-full border border-orange-200">Flujo de Caja Institucional</span>
            </div>
          </div>
          <nav className="flex items-center gap-4">
            ...
            {/* BOTÓN OBLIGATORIO: VOLVER A VISTA DE ALUMNO PERSONALES */}
            <button 
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              Panel Contable Centro Alumnos
            </h1>
            <p className="text-slate-500 mt-1">Supervisión de balances financieros y registro inmediato de transacciones.</p>
          </div>
          
          {/* BOTÓN DISPARADOR DEL MODAL DE GASTOS */}
          <button 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-900 p-6 rounded-xl shadow-sm border-none text-white">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-2 text-orange-400">
              <Wallet className="h-4 w-4" /> Capital Disponible CEA
            </p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50">
            <h3 className="font-bold text-slate-800">Libro de Cuentas CEA</h3>
          </div>
      ...
      {/* 🚀 MODAL FLOTANTE DE CREACIÓN DE GASTOS */}
      {isModalOpen && (
            ...
            <form onSubmit={handleSubmitGasto} className="space-y-4">
              ...
              <div>
                ...
                <input 
                  type="text" 
                  required
                  placeholder="Ej: Compra de balones para torneo recreativo"
                  value={descForm}
              ...
              <div>
                ...
                >
                  ...
                  <option value="Cultura">Cultura y Eventos</option>
                  <option value="Infraestructura">Infraestructura Escolar</option>
                  <option value="Donacion">Donaciones y Ayuda Social</option>
                </select>
              </div>
              ...
              >
                {submitLoading ? <Loader2 className="animate-spin h-5 w-5" /> : "Guardar en Oracle"}
              </button>
            </form>
          </div>
        </div>
