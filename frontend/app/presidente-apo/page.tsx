...
// --- INTERFACES SIMULADAS (Ajusta según tu backend) ---
...
export default function PresidenteApoderadoDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          if (payload.role !== "DIR_PRES_APO" && payload.rol !== "DIR_PRES_APO") {
            throw new Error("Acceso denegado: Vista exclusiva para Presidencia de Apoderados.");
          }
        ...
        // 2. Obtener datos financieros del curso (Ajusta estas rutas a tu API real)
        // Como es solo lectura, hacemos las peticiones en paralelo
        if (cursoIdDesdeToken) {
          /* NOTA: Reemplaza estas URLs con los endpoints reales de tu backend.
            Aquí uso URLs de ejemplo basadas en tu arquitectura anterior.
          */
          const [resResumen, resMovimientos, resCuotas] = await Promise.all([
            fetch(`${API_URL}/finanzas/curso/${cursoIdDesdeToken}/resumen`, { headers: headersAuth }).catch(() => null),
            fetch(`${API_URL}/finanzas/curso/${cursoIdDesdeToken}/movimientos`, { headers: headersAuth }).catch(() => null),
            fetch(`${API_URL}/finanzas/curso/${cursoIdDesdeToken}/cuotas`, { headers: headersAuth }).catch(() => null)
          ]);
          ...
          if (resResumen?.ok) {
            ...
            // Datos de fallback para visualización si la API aún no está lista
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="hidden md:inline-block ml-2 text-xs font-semibold bg-indigo-50 text-indigo-700 px-2.5 py-0.5 rounded-full border border-indigo-200">Presidencia Apoderados</span>
            </div>
          </div>
          <nav className="flex items-center gap-4">
            ...
            {/* BOTÓN CAMBIO DE VISTA A APODERADO */}
            <button 
              ...
              title="Volver al Portal de Apoderado"
            >
              <ArrowRightLeft className="h-4 w-4" />
              <span className="hidden sm:inline-block">Vista Apoderado</span>
            </button>
