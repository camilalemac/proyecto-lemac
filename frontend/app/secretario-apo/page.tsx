...
export default function SecretarioApoderadoDashboard() {
  const router = useRouter();

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  const [secretarioName, setSecretarioName] = useState<string>("Apoderado Secretario");
    ...
    const fetchDatosSecretario = async () => {
      try {
        ...
        try {
          ...
          // Ajusta "DIR_SEC_APO" al nombre real de tu rol si es diferente
          if (payload.role !== "DIR_SEC_APO" && payload.rol !== "DIR_SEC_APO") {
            throw new Error("Acceso denegado: Esta vista es exclusiva para la Secretaría de Apoderados.");
          }
        ...
        // 2. Obtener información de perfil (Opcional, para nombre bonito)
        try {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="hidden md:inline-block ml-2 text-xs font-semibold bg-blue-50 text-blue-700 px-2.5 py-0.5 rounded-full border border-blue-200">Portal Secretaría de Curso</span>
            </div>
          </div>
        ...
        <header className="print:hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Redacta los acuerdos de la reunión de apoderados e imprime el documento físico.
            </p>
          </div>
          <button 
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col print:border-none print:shadow-none print:rounded-none">
          ...
          <div className="p-6 print:hidden">
            <textarea
              ...
              placeholder="Escribe aquí el acta de la reunión... Ej: En Valparaíso, a 26 de mayo de 2026, se da inicio a la reunión de apoderados del curso..."
              value={actaText}
          ...
          <div className="hidden print:block print:p-8 print:w-full">
            <div className="text-center mb-8 border-b-2 border-slate-800 pb-4">
              <h1 className="text-2xl font-bold uppercase tracking-wider">Acta de Reunión de Apoderados</h1>
            ...
            <div className="mt-24 pt-8 flex justify-around items-end">
              ...
              <div className="text-center">
                <div className="w-48 border-t border-black mb-2"></div>
                <p className="font-semibold text-sm">Firma Secretario(a)</p>
              </div>
            </div>
          </div>
