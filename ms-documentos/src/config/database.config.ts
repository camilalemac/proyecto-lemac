...
const dbName =
  process.env.DB_NAME || (dialect === "mysql" ? "test" : "softwarepayescolar_reportes");
...
if (dialect === "oracle") {
  ...
  if (!fs.existsSync(tnsFile)) {
    throw new Error(`[ms-documentos] Oracle wallet no encontrado: ${tnsFile}`);
  }
  ...
  if (fs.existsSync(sqlnetFile)) {
    ...
    if (updatedSqlnetContent !== sqlnetContent) {
      ...
      logger.info(`[ms-documentos] sqlnet.ora actualizado con WALLET_LOCATION=${walletPath}`);
    }
  }
  ...
  try {
    ...
    logger.warn("[ms-documentos] oracledb.initOracleClient:", err);
  }
}

logger.info(`[ms-documentos] DB: dialect=${dialect}, name=${dbName}, user=${dbUser}`);

const sequelizeOptions: Record<string, unknown> = {
  dialect: dialect as "oracle" | "mysql",
  logging: (sql: string, timing?: number) => {
    logger.info(`[ms-documentos][sequelize] ${sql}${timing ? ` (${timing}ms)` : ""}`);
  },
